...
func (s *CSRFScanner) checkSameSite(cookies []*http.Cookie) {
...
// Check for CORS misconfigurations (related to CSRF/XSS)
func (s *CSRFScanner) checkCORS(resp *http.Response) {
	origin := resp.Header.Get("Access-Control-Allow-Origin")
	if origin == "*" {
		// Potential issue if credentials allowed
	}
}

/*
	Documentation:
	The CSRFScanner detects Cross-Site Request Forgery vulnerabilities.
...
func (s *CSRFScanner) SetConfig(cfg CSRFConfig) {
	// ...
}
